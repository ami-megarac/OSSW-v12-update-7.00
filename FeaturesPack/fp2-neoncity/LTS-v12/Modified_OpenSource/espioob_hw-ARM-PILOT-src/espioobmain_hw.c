...
static int espioob_irq,espioob_irq1;
...
int pilot_espioob_init(void)
{
    ...
    {
           espioob_irq = GetIrqFromDT("pilot_espioob_oobdummy", IRQ_SIO_PSR);
        if (request_irq(espioob_irq, oobdummy_irqhandler, IRQF_SHARED, "pilot_espioob_oobdummy", &oobdummy_irqhandler) < 0)
        {
    ...
    // Request IRQ of OOB
espioob_irq1 = GetIrqFromDT("pilot_espioob", IRQ_ESPI_OOB);
    if( request_irq( espioob_irq1, pilot_espioob_handler, IRQF_SHARED, "pilot_espioob", (pilot_espioob_virt_base + PILOT_ESPI_OOB_CHAN_ID)) < 0 )
    {
	printk(KERN_WARNING "failed to request irq %d", IRQ_ESPI_OOB);
        free_irq(espioob_irq, &oobdummy_irqhandler);
        unregister_hw_hal_module(EDEV_TYPE_ESPIOOB, pilot_espioob_hal_id);
        iounmap (pilot_espioob_virt_base);
        return -EBUSY;
    }
...
void pilot_espioob_exit(void)
{
    ...
    free_irq(espioob_irq1, (pilot_espioob_virt_base + PILOT_ESPI_OOB_CHAN_ID));
