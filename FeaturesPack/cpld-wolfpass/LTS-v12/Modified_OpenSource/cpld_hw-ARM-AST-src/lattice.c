...
uint32_t device_data_check(unsigned int* read_buffer, unsigned int* compare_data, unsigned int bits_leng ){
    ...
    if(compare_data != NULL)
    {
        for (i = 0; i < leng; i++) 
        {
            /* Reason for False Positive - All statements appear to serve a purpose in the function. No any logically dead */  
...
void read_code_size_data (unsigned int *JTAG_data_buffer,unsigned int TDI)
{
    ...
    unsigned short row_count, col_count; 
...
{
  uint32_t TDI[4] __attribute__ ((unused)) ={0}, TDO __attribute__ ((unused)) = 0;
  int ret = 0;
  int i = 0;
