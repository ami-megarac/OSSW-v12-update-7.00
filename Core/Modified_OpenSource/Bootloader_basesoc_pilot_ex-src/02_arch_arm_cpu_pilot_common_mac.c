...
+}
+
+static int ast_set_mac(struct eth_device *dev,unsigned long address)
+{
+	unsigned int maddr = dev->enetaddr[0] << 8 | dev->enetaddr[1];
+	unsigned int laddr = dev->enetaddr[2] << 24 | dev->enetaddr[3] << 16 | dev->enetaddr[4] << 8 | dev->enetaddr[5];
+	printf("\nmaddr=%x---laddr=%x\n",maddr,laddr);
+	writel(laddr,address+AST_MAC_CSR16);
+	writel(maddr,address+AST_MAC_CSR17);
+	return 0;
+}
