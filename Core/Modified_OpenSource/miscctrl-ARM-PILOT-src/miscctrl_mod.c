...
#else
static void power_timer_thread(struct timer_list *t)
#endif
{
    unsigned char icr = 0;
...
init_miscctrl_module(void)
{
   ...
   irq_rtc = GetIrqFromDT("ami_rtc", IRQ_RTC);


#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,2,0))
	if ((err = request_irq(irq_rtc, miscctrl_RTChandler, IRQF_DISABLED, "miscctrl", 0)) < 0)
#else
	if ((err = request_irq(irq_rtc, miscctrl_RTChandler, 0, "miscctrl", 0)) < 0)
#endif
	{
   ...
   irq_miscctrl = GetIrqFromDT("ami_miscctrl", IRQ_SIO_SWC);



#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,2,0))
	if ((err = request_irq(irq_miscctrl, miscctrl_SWMhandler, IRQF_DISABLED | IRQF_SHARED, "miscctrl", (void *)("miscctrl"))) < 0)
#else
	if ((err = request_irq(irq_miscctrl, miscctrl_SWMhandler, IRQF_SHARED, "miscctrl", (void *)("miscctrl"))) < 0)
#endif
	{
...
#else
			timer_setup(&power_timer,power_timer_thread,0);

            icr |= ENABLE_PWRBTOUT;
            *((unsigned char *)(SE_SYS_WAKEUP_VA_BASE+SLP_ST_REG)) = icr;
			mod_timer(&power_timer,jiffies + (miscctrl_field.timeout * HZ/1000));
#endif
        }
...
exit_miscctrl_module(void)
{
	...
	{
		free_irq(irq_miscctrl, (void *)("miscctrl"));
