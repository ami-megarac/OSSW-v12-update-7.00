...
#endif
{
	...
	{
		free_irq(reset_irq, (void*) ("lpcreset_irq"));
...
get_lpcreset_irq(void)
{
		...
		reset_irq = GetIrqFromDT("ami_reset", LPC_RST_IRQ_ADDR);


        #if (LINUX_VERSION_CODE >=  KERNEL_VERSION(2,6,24))
                err = request_irq(reset_irq, lpcreset_handler, IRQF_SHARED, "lpcreset", (void*)("lpcreset_irq"));
        #else
                err = request_irq(reset_irq, lpcreset_handler, SA_SHIRQ, "lpcreset", (void*)("lpcreset_irq"));
        #endif

        if (err < 0)
                printk("failed to request irq %d (err %d)", reset_irq, err);
