...
#endif
{
    ...
    irq_gpio = GetIrqFromDT("ami_gpio", IRQ_GPIO);
...
#else
static void GpioToggleTimerFunc(struct timer_list *t)
#endif
{
	unsigned char loop;
...
int pilot_EnableGpioToggle( void* paramToggleData)
{
	...
	if(!SoftGpioToggle_IsTimerRunning())
	{
		...
		add_timer(&ToggleTimer);	
#else
	 	timer_setup(&ToggleTimer,GpioToggleTimerFunc,0);
      	mod_timer(&ToggleTimer,jiffies + (pData->ToggleCycle /10));
#endif
