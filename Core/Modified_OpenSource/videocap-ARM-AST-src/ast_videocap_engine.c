...
Redo:
	...
	if (((reg & 0xff00) >> 8) == 0xA8) { // Driver supports to get display information in scratch register
		...
		if(CaptureMode == VIDEOCAP_YUV_SUPPORT) //Ref: (sdk7.1) drivers/soc/aspeed/ast_video.c > ast_video_vga_mode_detect()
		{ // For JViewer/H5Viewer
			...
			else /* Enable direct mode for 15 bpp and higher */
			{
				...
				/* Disable direct mode to fix engine busy issue in win2019 host(1360x768x60hz) */
				if ((info->src_mode.x * info->src_mode.y) == (1360 * 768))
					info->INFData.DirectMode = 0;
				else
