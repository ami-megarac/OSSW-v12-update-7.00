...
pwmtach_ioctlUnlocked (struct file *file, uint cmd, ulong arg)
{
	...
	in_data.fanmap_dataptr = NULL;
	in_data.fanproperty_dataptr = NULL;
	...
	if( ((cmd == CONFIGURE_FANMAP_TABLE) || (cmd == CONFIGURE_FANPROPERTY_TABLE)) && (in_data.num_fans > 0) )
	{
		if( in_data.num_fans >= 100 )
		{
			printk("Invalid fan counts to allocate memory: %d\n", in_data.num_fans);
			return -1;
		}
		count = sizeof(struct fan_map_entry_t) * in_data.num_fans;
		in_data.fanmap_dataptr = kmalloc( count, GFP_KERNEL );
		if( !in_data.fanmap_dataptr )
		{
			printk("Allocate memory for Fan Map failed!\n");
			return -1;
		}
		count = sizeof(struct fan_property_t) * in_data.num_fans;
		in_data.fanproperty_dataptr = kmalloc( count, GFP_KERNEL );
		if( !in_data.fanproperty_dataptr )
		{
			printk("Allocate memory for Fan Property failed!\n");
			goto LBL_FailExit;
		}
	}
...
/***************************************************************************/
		...
		case CONFIGURE_FANMAP_TABLE:
			count = sizeof(struct fan_map_entry_t) * in_data.num_fans;
			pDesc = (void*)(in_data.fanmap_dataptr);
			pSrc = (void *)(((pwmtach_data_t*)arg)->fanmap_dataptr);
			dwResult = copy_from_user(pDesc, pSrc, count);
			if ( dwResult != 0 )
			{
				iRet = -EFAULT;
				goto LBL_Exit;
			}
			if( (in_data.fanproperty_dataptr != NULL) && (in_data.fanmap_dataptr != NULL)  )
			{
				iRet = configure_fanmap_table (pdev, &in_data);
				if ( iRet != 0 )
					goto LBL_Exit;
				if ( copy_to_user(pSrc, pDesc, count) )
				{
					iRet = -EFAULT;
					goto LBL_Exit;
				}
		...
		case CONFIGURE_FANPROPERTY_TABLE:
			count = sizeof(struct fan_property_t) * in_data.num_fans;
			pDesc = (void*)(in_data.fanproperty_dataptr);
			pSrc = (void *)(((pwmtach_data_t*)arg)->fanproperty_dataptr);
			dwResult = copy_from_user(pDesc, pSrc, count);
			if ( dwResult != 0 )
			{
				iRet = -EFAULT;
			...
			if( (in_data.fanproperty_dataptr != NULL) && (in_data.fanmap_dataptr != NULL) )
			{
				iRet = configure_fanproperty_table (pdev, &in_data);
				if ( iRet != 0 )
					goto LBL_Exit;
				if ( copy_to_user(pSrc, pDesc, count) )
				{
	...
	count  = sizeof(pwmtach_data_t) - (sizeof(void*) * 2);
	if ( copy_to_user((void*)arg, &in_data, count) )
	{
		iRet = -EFAULT;
		goto LBL_Exit;
	}
...
LBL_Exit:
	if( !in_data.fanmap_dataptr )
	{
		kfree( in_data.fanmap_dataptr );
		in_data.fanmap_dataptr = NULL;
	}
	if( !in_data.fanproperty_dataptr )
	{
		kfree( in_data.fanproperty_dataptr );
		in_data.fanproperty_dataptr = NULL;
	}
