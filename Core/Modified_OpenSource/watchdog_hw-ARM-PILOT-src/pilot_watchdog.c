...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,2,0))				/* TODO: Remove once spi core has this function */
...
static int __init pilot_watchdog_init(void)
{
        ...
        if (!pilot_watchdog_virt_base) {
   		...
   		watchdog_irq = GetIrqFromDT("ami_wdt", PILOT_WATCHDOG_IRQ);

#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,2,0))
        ret = request_irq(watchdog_irq, pilot_watchdog_irq_handler, IRQF_DISABLED, PILOT_WATCHDOG_DRIVER_NAME, pilot_watchdog_virt_base);
#else
        ret = request_irq(watchdog_irq, pilot_watchdog_irq_handler, 0, PILOT_WATCHDOG_DRIVER_NAME, pilot_watchdog_virt_base);
#endif
        if (ret) {
...
{
        free_irq(watchdog_irq, pilot_watchdog_virt_base);
        iounmap(pilot_watchdog_virt_base);
        release_mem_region(PILOT_WATCHDOG_REG_BASE, PILOT_WATCHDOG_REG_LEN);
        unregister_hw_hal_module(EDEV_TYPE_WATCHDOG, pilot_watchdog_hal_id);
}
