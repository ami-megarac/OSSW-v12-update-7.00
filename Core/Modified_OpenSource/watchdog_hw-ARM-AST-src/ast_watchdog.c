...
static struct watchdog_core_funcs_t *watchdog_core_ops_new;
static int ast_watchdog_hal_id;
static int ast_watchdog_hal_id_new;
...
static void ast_watchdog_set_value_new(int value, int dev_instance)
{
#ifdef CONFIG_SPX_FEATURE_SELECT_WDT2
        iowrite32(value * AST_WATCHDOG_COUNT_PER_SEDOND, ast_watchdog_virt_base + AST_WATCHDOG2_VALUE);
#else
        iowrite32(value * AST_WATCHDOG_COUNT_PER_SEDOND, ast_watchdog_virt_base + (dev_instance * 0x20) + AST_WATCHDOG_VALUE);
#endif
}
...
static void ast_watchdog_set_pretimeout_new(int dev_instance)
{
	uint32_t reg = 0;
#ifdef CONFIG_SPX_FEATURE_SELECT_WDT2
	/* pre-timeout set 1 second */
...
#else
	/* pre-timeout set 1 second */
	reg = ioread32( (void __iomem*)ast_watchdog_virt_base + (dev_instance * 0x20) + AST_WATCHDOG_CTRL);
	reg = reg | AST_WATCHDOG_PRETIMEOUT_1S;
    	reg = reg | AST_WATCHDOG_CTRL_INTERRUPT;
    	iowrite32(reg, (void __iomem*)ast_watchdog_virt_base + (dev_instance * 0x20) + AST_WATCHDOG_CTRL);
#endif
}
...
static void ast_watchdog_clear_pretimeout_new(int dev_instance)
{
	uint32_t reg = 0;
#ifdef CONFIG_SPX_FEATURE_SELECT_WDT2
	/* pre-timeout set 1 second */
...
static int ast_watchdog_get_status_new(int dev_instance) 
{ 
    /* return watchdog timeout status to determine system reset from 2nd boot source or not */ 
    return ((ioread32(ast_watchdog_virt_base + AST_WATCHDOG_TIMEOUT + (dev_instance * 0x20)) & 0x00000002) >> 1 ); 
} 
...
static void ast_watchdog_count_new(int dev_instance)
{
#ifdef CONFIG_SPX_FEATURE_SELECT_WDT2
        iowrite32(AST_WATCHDOG_RELOAD_MAGIC, ast_watchdog_virt_base + AST_WATCHDOG2_RESTART);
#else
        iowrite32(AST_WATCHDOG_RELOAD_MAGIC, ast_watchdog_virt_base + (dev_instance * 0x20) + AST_WATCHDOG_RESTART);
#endif
}
...
static void ast_watchdog_enable_new(int dev_instance)
{
        /* we use external 1MHz clock source */
#ifdef CONFIG_SPX_FEATURE_SELECT_WDT2
        iowrite32(AST_WATCHDOG_CTRL_CLOCK | AST_WATCHDOG_CTRL_RESET | AST_WATCHDOG_CTRL_ENABLE, ast_watchdog_virt_base + AST_WATCHDOG2_CTRL);
#else
        iowrite32(AST_WATCHDOG_CTRL_CLOCK | AST_WATCHDOG_CTRL_RESET | AST_WATCHDOG_CTRL_ENABLE, ast_watchdog_virt_base + (dev_instance * 0x20) + AST_WATCHDOG_CTRL);
#endif
#if defined(SOC_AST2500)
        iowrite32(g_wdtdev_reset_mask[dev_instance], ast_watchdog_virt_base + (dev_instance * 0x20) + AST_WATCHDOG_RESET_MASK);
#endif  
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,2,0))
	ast_watchdog_set_pretimeout_new(dev_instance);
#endif
}
...
static void ast_watchdog_disable_new(int dev_instance)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,2,0))
    #ifdef CONFIG_SPX_FEATURE_SELECT_WDT2
        iowrite32(ioread32(ast_watchdog_virt_base + AST_WATCHDOG2_CTRL) & ~(AST_WATCHDOG_CTRL_ENABLE | AST_WATCHDOG_CTRL_INTERRUPT), ast_watchdog_virt_base + AST_WATCHDOG2_CTRL);
    #else
        iowrite32(ioread32(ast_watchdog_virt_base + AST_WATCHDOG_CTRL) & ~(AST_WATCHDOG_CTRL_ENABLE | AST_WATCHDOG_CTRL_INTERRUPT), ast_watchdog_virt_base + (dev_instance * 0x20) + AST_WATCHDOG_CTRL);
    #endif
    ast_watchdog_clear_pretimeout_new(dev_instance);
#else
    #ifdef CONFIG_SPX_FEATURE_SELECT_WDT2
        iowrite32(ioread32(ast_watchdog_virt_base + AST_WATCHDOG2_CTRL) & ~AST_WATCHDOG_CTRL_ENABLE, ast_watchdog_virt_base + AST_WATCHDOG2_CTRL);
    #else
        iowrite32(ioread32(ast_watchdog_virt_base + AST_WATCHDOG_CTRL) & ~AST_WATCHDOG_CTRL_ENABLE, ast_watchdog_virt_base + (dev_instance * 0x20) + AST_WATCHDOG_CTRL);
    #endif
#endif
}
...
static struct watchdog_hal_ops_new_t ast_ops_new = {
        .set_value = ast_watchdog_set_value_new,
        .count = ast_watchdog_count_new,
        .get_status = ast_watchdog_get_status_new,
        .enable = ast_watchdog_enable_new,
        .disable = ast_watchdog_disable_new,
};
...
static hw_hal_t ast_hal_new = {
        .dev_type = EDEV_TYPE_WATCHDOG_NEW,
        .owner = THIS_MODULE,
        .devname = AST_WATCHDOG_DRIVER_NAME,
        .num_instances = MAX_WDT_DEVICES,
        .phal_ops = (void *) &ast_ops_new
};
...
static int __init ast_watchdog_init(void)
{
        ...
        ast_watchdog_hal_id_new = register_hw_hal_module(&ast_hal_new, (void **) &watchdog_core_ops_new);
        if (ast_watchdog_hal_id_new < 0) {
                printk(KERN_WARNING "%s (New): register HAL HW module failed\n", AST_WATCHDOG_DRIVER_NAME);
                return ast_watchdog_hal_id_new;
        }
...
#endif
        iowrite32(ioread32(ast_watchdog_virt_base + AST_WATCHDOG2_CTRL) & ~AST_WATCHDOG_CTRL_ENABLE, ast_watchdog_virt_base + AST_WATCHDOG2_CTRL);
        iowrite32(0, ast_watchdog_virt_base + AST_WATCHDOG_CTRL);
        iowrite32(WDT_CTRL_RESET_MODE_SOC | WDT_CTRL_RESET_SYSTEM, ast_watchdog_virt_base + AST_WATCHDOG_CTRL);
       
        reg = ioread32(ast_watchdog_virt_base + AST_WATCHDOG_WIDTH);
        reg &= 0x0000FFFF;
        iowrite32(reg | WDT_ACTIVE_LOW_MAGIC, ast_watchdog_virt_base + AST_WATCHDOG_WIDTH);
       
        reg &= 0x0000FFFF;  
        iowrite32(reg | WDT_OPEN_DRAIN_MAGIC, ast_watchdog_virt_base + AST_WATCHDOG_WIDTH);
...
out_hal_register:
        ...
        unregister_hw_hal_module(EDEV_TYPE_WATCHDOG_NEW, ast_watchdog_hal_id_new);
...
static void __exit ast_watchdog_exit(void)
{
    int cnt = 0;
        ...
        for(cnt=0; cnt<MAX_WDT_DEVICES; cnt++) {
    		ast_watchdog_disable_new(cnt);
        }
