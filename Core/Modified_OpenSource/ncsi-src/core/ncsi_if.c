...
ncsi_ioctlUnlocked (struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	memset(&ncsi_ioctl_data,0,sizeof(ncsi_ioctl_data_T));
	...
	switch( cmd )
	{
		...
		case USER_CMD:    
			...
			/* Reason for False Positive - String always has value, other has filtered above if function. */
			/* coverity[string_null : FALSE] */
			length=strlen(ncsi_ioctl_data.interface_name);
			ncsi_ioctl_data.interface_name[length]='\0';
			info = GetInterfaceInfoByName(ncsi_ioctl_data.interface_name);
			if (info != NULL)
			{
				/* Reason for False Positive - This function is create for user. */
				/* coverity[user_pointer : FALSE] */
