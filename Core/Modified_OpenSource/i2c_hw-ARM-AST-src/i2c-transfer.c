                    ...
                    
{
	...
	case I2C_SMBUS_BLOCK_DATA:
		if (read_write == I2C_SMBUS_READ) {
			...
			as_data_ptr[i2c_adap->nr].block_read = 0;
			as_data_ptr[i2c_adap->nr].block_proc_call= 0;
			as_data_ptr[i2c_adap->nr].cmdsent = msgbuf0[0];
		}
		break;
...
retry:
	...
	if (as_data_ptr[i2c_adap->nr].MCTPRX_Entries)
	{
		...
		if (len > 0)
		{
			...
			if(as_data_ptr[i2c_adap->nr].MCTPRX_data[FifoPtr][1] != MCTP_CMD_CODE)
			{
				buf[0] = as_data_ptr[i2c_adap->nr].MCTPRX_data[FifoPtr][0];
				buf[1] = MCTP_CMD_CODE;
				memcpy(&buf[2],&as_data_ptr[i2c_adap->nr].MCTPRX_data[FifoPtr][1],len - 1);
				len++;
			}
			else
		...
		if( (i2c_as_read_reg(i2c_adap->nr,I2C_FUNCTION_CONTROL_REG) & ENABLE_SLAVE_FUNC ) == 0)
   ...
   else
	{
  	 	spin_unlock_irqrestore( &as_data_ptr[i2c_adap->nr ].data_lock , flags);
		#ifdef AMI_TEST
		as_data_ptr[i2c_adap->nr].MCTPFlag |= 0x0001;
		#endif
		ret = wait_event_interruptible_timeout(as_data_ptr[i2c_adap->nr].as_mctp_data_wait,as_data_ptr[i2c_adap->nr].MCTPRX_Entries, HZ);
		while(as_data_ptr[i2c_adap->nr].packetComing)//wait for packets from a master
		{
			as_data_ptr[i2c_adap->nr].packetComing = 0;
			if(cnt < 2)
				mdelay (1);
			else	
				mdelay (5);
	...
	if (len <=0)
		goto retry;
#ifdef AMI_TEST
	if(as_data_ptr[i2c_adap->nr].packetComing == 0)
    	as_data_ptr[i2c_adap->nr].MCTPFlag &= 0xFFFE;
#endif
	return (len==MAX_IPMB_MSG_SIZE)?0:len;
}
...
{ 
	...
	log_i2c_plain(i2c_adap, buf, size);
