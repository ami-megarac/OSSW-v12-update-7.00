...
#ifdef AMI_TEST		
		if(as_data_ptr[bus].MCTPFlag & 0x0001)
		{
//		as_data_ptr[bus].SlaveRX_index[FifoPtr] = 0;
		    DataBuffer = as_data_ptr[bus].MCTPRX_data[as_data_ptr[bus].MCTPRX_Writer];
		    DataBuffer[0] = (i2c_as_read_reg( bus, I2C_SLAVE_ADDR_REG ) << 1);
		    for (i=1; i < datacount ;i++)
			{
				DataBuffer[as_data_ptr[bus].Linear_SlaveRX_index + i] = as_data_ptr[bus].dma_buff[i-1];
			}
			if( as_data_ptr[bus].MCTPRX_Len[as_data_ptr[bus].MCTPRX_Writer] == 0)
			{
		    	as_data_ptr[bus].Linear_SlaveRX_data[3] = DataBuffer[3];
		    	as_data_ptr[bus].Linear_SlaveRX_data[1] = DataBuffer[1];
		    }
		    as_data_ptr[bus].MCTPRX_Len[as_data_ptr[bus].MCTPRX_Writer] += datacount;
		    as_data_ptr[bus].Linear_SlaveRX_index += datacount;
		    as_data_ptr[bus].packetComing = 1
		}
		else
		{
			as_data_ptr[bus].Linear_SlaveRX_data[0] = (i2c_as_read_reg( bus, I2C_SLAVE_ADDR_REG ) << 1);
...
#ifdef AMI_TEST	
		if((as_data_ptr[bus].MCTPFlag & 0x0001 )== 0)
		{
#endif
...
as_handler(int bus)
{
    ...
    /*Reason for False Positive - Set bits to disable the I2C Bus. (Ex: bit 1 = Bus 1)*/
