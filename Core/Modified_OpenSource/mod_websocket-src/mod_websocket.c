...
# undef	N
    ...
    for (i = hdrs->used; i > 0; i--) {
        ...
        if (buffer_is_equal_string(hdr->key, CONST_STR_LEN("Connection"))) {
            /* Reason for false positive - The memory allocated for connection_hdr_value is being freed before returning handler_t */
        ...
        if (buffer_is_equal_string(hdr->key, CONST_STR_LEN("Upgrade"))) {
            /* Reason for false positive - The memory allocated for upgrade_hdr_value is being freed before returning handler_t */
            /* coverity[overwrite_var : FALSE] */
