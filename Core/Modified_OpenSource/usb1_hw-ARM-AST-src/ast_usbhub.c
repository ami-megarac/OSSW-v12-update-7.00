...
void ast_usbhub_enable_dev(uint8_t dev_num, uint8_t speed)
{
	...
	if (AST_USBHUB_DEV_NUM <= dev_num)	// SCA Fix to avoid "Out-of-bounds read"/"Negative array index read"/"Out-of-bounds write"/"Out-of-bounds access" occurred when return -1 with ast_usbhub_convert_dev_to_port().
	{
		return ;
	}
...
void ast_usbhub_disable_dev(uint8_t dev_num, uint8_t speed)
{
	...
	if (AST_USBHUB_DEV_NUM <= dev_num)	// SCA Fix to avoid "Negative array index read"/"Out-of-bounds write" occurred when return -1 with ast_usbhub_convert_dev_to_port().
	{
		return ;
	}
...
int ast_usbhub_stall_ep(uint8_t dev_num, uint8_t ep_num, uint8_t ep_dir)
{
	...
	if (AST_USBHUB_DEV_NUM <= dev_num)	// SCA Fix to avoid "Out-of-bounds read"/"Negative array index read" occurred when return -1 with ast_usbhub_convert_dev_to_port().
	{
		return -EINVAL;
	}
...
int ast_usbhub_write_data_to_host(uint8_t dev_num, uint8_t ep_num, uint8_t *buf, uint16_t len)
{
	...
	if (AST_USBHUB_DEV_NUM <= dev_num)	// SCA Fix to avoid "Out-of-bounds read"/"Negative array index read"/"Out-of-bounds write" occurred when return -1 with ast_usbhub_convert_dev_to_port().
	{
		TCRIT("overflow with dev_num\n");
		return -EINVAL;
	}
...
void ast_usbhub_set_addr(uint8_t dev_num, uint8_t addr, uint8_t enable)
{
	...
	if (AST_USBHUB_DEV_NUM <= dev_num)	// SCA Fix to avoid "Out-of-bounds read"/"Out-of-bounds write" occurred when return -1 with ast_usbhub_convert_dev_to_port().
	{
		return ;
	}
...
void ast_usbhub_get_addr(uint8_t dev_num, uint8_t *addr, uint8_t *enable)
{
	...
	if (AST_USBHUB_DEV_NUM <= dev_num)	// SCA Fix to avoid "Negative array index read"/"Out-of-bounds read" occurred when return -1 with ast_usbhub_convert_dev_to_port().
	{
		TCRIT("overflow with dev_num\n");
		return ;
	}
...
{
	dev_num = ast_usbhub_convert_dev_to_port(dev_num);
	if (AST_USBHUB_DEV_NUM <= dev_num)	// SCA Fix to avoid "Out-of-bounds write" occurred when return -1 by ast_usbhub_convert_dev_to_port().
		return;
