...
int phy_espi_irq[ PILOT_ESPI_CHANNEL_NUM ] =   /* Raw IRQ */
{
    PILOT_ESPI_IRQ,
    IRQ_ESPI_VWIRE,
};

char *name_espi_irq[ PILOT_ESPI_CHANNEL_NUM ] =   /* Device tree name for irq */
{
	"ami_espi",
	"ami_espi_vwire",
};

static int espi_irq[ PILOT_ESPI_CHANNEL_NUM ];	  /* Mapped IRQ */
static int irq_sio_psr;
...
static void pilot_espi_hw_init(void) 
{
   ...
   // espi hardware initialization
    ...
    {
 		espi_irq[i] = GetIrqFromDT(name_espi_irq[i], phy_espi_irq[i]);

#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,2,0))
        if( request_irq( espi_irq[i], espi_irq_handlers[i], IRQF_DISABLED, "pilot_espi", (pilot_espi_virt_base + i)) < 0 )
#else
        if( request_irq( espi_irq[i], espi_irq_handlers[i], 0, "pilot_espi", (pilot_espi_virt_base + i)) < 0 )
#endif
            printk(KERN_WARNING "%s: request irq %d failed\n", PILOT_ESPI_DRIVER_NAME, espi_irq[i]);
    }

	irq_sio_psr = GetIrqFromDT("ami_espi_reset", IRQ_SIO_PSR);
