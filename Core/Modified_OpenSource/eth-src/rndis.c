...
static uint32 OIDListSupported[] = 
{
	...
	OID_GEN_SUPPORTED_GUIDS	// For HLK test with Server 2022: oidswmi.
};
...
//		*pDATA32 = cpu_to_le32(Info->Speed);
	...
	case OID_GEN_SUPPORTED_GUIDS:	// For HLK test with Server 2022: oidswmi.
		
		if(AllocCmplt(sizeof(NDIS_GUID)) != NULL)
			memcpy(pDATA8, (uint8*) &dummyGUID, sizeof(NDIS_GUID));
		break;
