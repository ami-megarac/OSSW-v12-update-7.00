...
#endif
{
    ...
    kcs_irq = GetIrqFromDT("ami_kcs", LPC_IRQ_ADDR);

#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,2,0))
#if (LINUX_VERSION_CODE >=  KERNEL_VERSION(2,6,24)) 
	if ((ret = request_irq(kcs_irq, kcs_hw_irq_handler, IRQF_DISABLED | IRQF_SHARED, "kcs", (void*)kcs_hw_irq_handler)) != 0)
#else
	if ((ret = request_irq(kcs_irq, kcs_hw_irq_handler, SA_INTERRUPT, "kcs", 0)) != 0)
#endif
#else
  if ((ret = request_irq(kcs_irq, kcs_hw_irq_handler, IRQF_SHARED, "kcs", (void*)kcs_hw_irq_handler)) != 0)
#endif
	{
		printk (KERN_ERR "%s: failed to request irq %d (err %d)\n", KCS_HW_DEV_NAME, kcs_irq, ret);
		ret = -ENODEV;
		goto fail;
	}
...
#endif
{
  ...
  for(ch = 0;ch < MAX_KCS_HW_CHANNELS;ch ++)
  {
	...
	{
#if (LINUX_VERSION_CODE >=  KERNEL_VERSION(2,6,24)) 
		free_irq(kcs_irq, (void*)kcs_hw_irq_handler);
#else
		free_irq(kcs_irq, 0);
#endif
