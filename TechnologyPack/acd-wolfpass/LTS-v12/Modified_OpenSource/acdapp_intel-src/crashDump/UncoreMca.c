...
static void uncoreMcaJson(UINT8 u8Cpu, SUncoreMcaRawData * sUncoreMcaRawData, cJSON * pJsonChild)
{
    ...
    for (i = FIRST_UNCORE_MCA; i <= LAST_UNCORE_MCA; i++) {
        ...
        if (sUncoreMcaRawData[i].bInvalid) {
			snprintf(jsonItemString, UNCORE_MCA_JSON_STRING_LEN, "%s0x%02x", UA_STRING, sUncoreMcaRawData[i].uRegData.u32Raw[0]);
		...
		if (sUncoreMcaRawData[i].u8CC[0] == PECI_CC_SUCCESS) {
			snprintf(jsonItemString, UNCORE_MCA_JSON_STRING_LEN, "0x%08x%08x",
				sUncoreMcaRawData[i].uRegData.u32Raw[1], sUncoreMcaRawData[i].uRegData.u32Raw[0]);
		}
		else {
			snprintf(jsonItemString, UNCORE_MCA_JSON_STRING_LEN, "0x%08x%08x,%s0x%02x",
				sUncoreMcaRawData[i].uRegData.u32Raw[1], sUncoreMcaRawData[i].uRegData.u32Raw[0],
				UA_STRING, sUncoreMcaRawData[i].u8CC[0]);
		}
        cJSON_AddStringToObject(uncoreMca, uncoreMcaRegNames[UNCORE_CTL], jsonItemString);

		if (sUncoreMcaRawData[i].u8CC[1] == PECI_CC_SUCCESS) {
			snprintf(jsonItemString, UNCORE_MCA_JSON_STRING_LEN, "0x%08x%08x",
				sUncoreMcaRawData[i].uRegData.u32Raw[3], sUncoreMcaRawData[i].uRegData.u32Raw[2]);
		}
		else {
			snprintf(jsonItemString, UNCORE_MCA_JSON_STRING_LEN, "0x%08x%08x,%s0x%02x",
				sUncoreMcaRawData[i].uRegData.u32Raw[3], sUncoreMcaRawData[i].uRegData.u32Raw[2],
				UA_STRING, sUncoreMcaRawData[i].u8CC[1]);
		}
		cJSON_AddStringToObject(uncoreMca, uncoreMcaRegNames[UNCORE_STATUS], jsonItemString);

		if (sUncoreMcaRawData[i].u8CC[2] == PECI_CC_SUCCESS) {
			snprintf(jsonItemString, UNCORE_MCA_JSON_STRING_LEN, "0x%08x%08x",
				sUncoreMcaRawData[i].uRegData.u32Raw[5], sUncoreMcaRawData[i].uRegData.u32Raw[4]);
		}
		else {
			snprintf(jsonItemString, UNCORE_MCA_JSON_STRING_LEN, "0x%08x%08x,%s0x%02x",
				sUncoreMcaRawData[i].uRegData.u32Raw[5], sUncoreMcaRawData[i].uRegData.u32Raw[4],
				UA_STRING, sUncoreMcaRawData[i].u8CC[2]);
		}
		cJSON_AddStringToObject(uncoreMca, uncoreMcaRegNames[UNCORE_ADDR], jsonItemString);

		if (sUncoreMcaRawData[i].u8CC[3] == PECI_CC_SUCCESS) {
			snprintf(jsonItemString, UNCORE_MCA_JSON_STRING_LEN, "0x%08x%08x",
				sUncoreMcaRawData[i].uRegData.u32Raw[7], sUncoreMcaRawData[i].uRegData.u32Raw[6]);
		}
		else {
			snprintf(jsonItemString, UNCORE_MCA_JSON_STRING_LEN, "0x%08x%08x,%s0x%02x",
				sUncoreMcaRawData[i].uRegData.u32Raw[7], sUncoreMcaRawData[i].uRegData.u32Raw[6],
				UA_STRING, sUncoreMcaRawData[i].u8CC[3]);
		}
		cJSON_AddStringToObject(uncoreMca, uncoreMcaRegNames[UNCORE_MISC], jsonItemString);

		if (sUncoreMcaRawData[i].u8CC[4] == PECI_CC_SUCCESS) {
			snprintf(jsonItemString, UNCORE_MCA_JSON_STRING_LEN, "0x%08x%08x",
				sUncoreMcaRawData[i].uRegData.u32Raw[9], sUncoreMcaRawData[i].uRegData.u32Raw[8]);
		}
		else {
			snprintf(jsonItemString, UNCORE_MCA_JSON_STRING_LEN, "0x%08x%08x,%s0x%02x",
				sUncoreMcaRawData[i].uRegData.u32Raw[9], sUncoreMcaRawData[i].uRegData.u32Raw[8],
				UA_STRING, sUncoreMcaRawData[i].u8CC[4]);
		}
		cJSON_AddStringToObject(uncoreMca, uncoreMcaRegNames[UNCORE_CTL2], jsonItemString);
    }
}
...
ESTATUS logUncoreMca(FILE * fpRaw, FILE * fp, cJSON * pJsonChild)
{
    ...
    for (u8Cpu = CPU0_ID; u8Cpu < MAX_CPU; u8Cpu++) {
		...
		if (u8Dword % 2 == 0)
			sUncoreMcaRawData[j].u8CC[u8Dword / 2] = PECI_CC_SUCCESS;
		...
		if (ePECIStatus != PECI_CC_SUCCESS) {
			...
			sUncoreMcaRawData[j].u8CC[u8Dword / 2] = sRdPkgConfigRes.u8CompletionCode;
