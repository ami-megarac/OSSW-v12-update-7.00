...
#endif
{
    ...
    switch (pSPlatDBG->ePlatDBGCmd) {
	    ...
	    for (i = 0; i < debugLogCount; i++) {
            ...
            if (NULL != fp) {
				/*Reason for False Positive –the fp will be assigned file point, not always is null */
...
int main (int argc, char **argv)
{
	...
	int validTrigger = 0, checkErrCount = 0, tmp = 0;
	...
	if (stat(crashdump_json_dir, &st) == -1) {
		tmp = mkdir(crashdump_json_dir, 0700);
        if(tmp == -1)
        {
            printf("cannot create folder \n");
        }
...
/*        if( access( "/var/crashdump/json/sysdebug2.json", F_OK ) != -1 ) {
    ...
    tmp = system("/usr/local/bin/acdbafigenerator -t /var/crashdump/json/crashdump.json");
    if(tmp == -1)
    {
        printf("BAFI can be run \n");
    }
#endif
	tmp = system("/etc/init.d/crashdump_compress.sh &");
    if(tmp == -1)
    {
        printf("ACD compress fails \n");
    }
#ifdef CONFIG_SPX_FEATURE_GLOBAL_AUTONOMOUS_CRASH_DUMP_DECODE_SUPPORT
	tmp = system("/etc/init.d/crashdump_decode.sh &");
    if(tmp == -1)
    {
        printf("ACD decode fails \n");
    }
#endif
 ...
 /*       if( access( "/var/crashdump/json/sysdebug2.json", F_OK ) != -1 ) {
        ...
        /*Reason for False Positive - no need to check this function*/
