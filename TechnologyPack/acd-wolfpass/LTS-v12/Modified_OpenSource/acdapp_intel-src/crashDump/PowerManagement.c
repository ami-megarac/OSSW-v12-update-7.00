...
ESTATUS logPowerManagement(FILE * fpRaw, FILE * fp, cJSON * pJsonChild)
{
	...
	PRINT(PRINT_DBG, PRINT_INFO, "Platform Debug - Power Management %d\n", u8Cpu);
...
#endif
        if (sRdPkgConfigRes.u8CompletionCode != PECI_CC_SUCCESS) {
           PRINT(PRINT_DBG, PRINT_ERROR, "[ERROR] - Failed to get number of cores for CPU #%d (cc=0x%x)\n", u8Cpu, sRdPkgConfigRes.u8CompletionCode);
           u32NumCores = 0;
#ifdef BUILD_RAW
	    fwrite(&u32NumCores, sizeof(u32NumCores), 1, fpRaw);
#endif //BUILD_RAW
        ...
        // chop off the thread count on 0x91/0x94 read if exceed the max
	if (sRdPkgConfigRes.u8CompletionCode == PECI_CC_MAYBE_SUCCESS || sRdPkgConfigRes.u8CompletionCode == PECI_CC_CMD_PASSED) {
        ...
        PRINT(PRINT_DBG, PRINT_INFO, "[INFO] - CPU #%d has %ld cores (cc=0x%x)\\n", u8Cpu, u32NumCores, sRdPkgConfigRes.u8CompletionCode);
