...
static void addressMapJson(SAddrMapRawData * sAddrMapRawData, cJSON * pJsonChild)
{
    ...
    for (i = 0; i < (sizeof(sSysMem) / sizeof(SSysMemRegion)) - 1; i++) {
        ...
        for (u8Reg = 0; u8Reg < sSysMem[i].u8NumRegs; u8Reg++) {
			if (sAddrMapRawData->sysAddr[u32SysAddrIndex].u8CC == PECI_CC_SUCCESS)
				...
				// Add PECI completion code in JSON if not 0x40
				snprintf(jsonItemString, AM_JSON_STRING_LEN, "%08x%08x,%s0x%02x", 
					     sAddrMapRawData->sysAddr[u32SysAddrIndex].uValue.u32[1], 
					     sAddrMapRawData->sysAddr[u32SysAddrIndex].uValue.u32[0],
					     UA_STRING,
					     sAddrMapRawData->sysAddr[u32SysAddrIndex].u8CC);
    ...
    for (u8Cpu = 0; u8Cpu < MAX_CPU; u8Cpu++) {
        ...
        for (i = 0; i < AM_NUM_MMIO; i++) {
			...
			if (sAddrMapRawData->mmioAddr[u8Cpu][i].u8CC == PECI_CC_SUCCESS)
				...
				// Add PECI completion code in JSON if not 0x40
				snprintf(jsonItemString, AM_JSON_STRING_LEN, "%08x%08x,%s0x%02x", 
					     sAddrMapRawData->mmioAddr[u8Cpu][i].uValue.u32[1],
					     sAddrMapRawData->mmioAddr[u8Cpu][i].uValue.u32[0],
					     UA_STRING,
					     sAddrMapRawData->mmioAddr[u8Cpu][i].u8CC);
...
ESTATUS logAddressMap(FILE * fpRaw, FILE * fp, cJSON * pJsonChild)
{
...
#ifndef SPX_BMC_ACD
           { "TSEG0",          AM_LOCAL_BUS_0, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     TSEG,           AM_REG_QWORD,  PECI_CC_SUCCESS},
            { "TSEG1",          AM_LOCAL_BUS_1, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     TSEG,           AM_REG_QWORD,  PECI_CC_SUCCESS},
            { "TSEG2",          AM_LOCAL_BUS_2, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     TSEG,           AM_REG_QWORD,  PECI_CC_SUCCESS},
            { "TSEG3",          AM_LOCAL_BUS_3, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     TSEG,           AM_REG_QWORD,  PECI_CC_SUCCESS},
            { "MESEG_BASE",     AM_LOCAL_BUS_1, AM_DEV_CHA_29,  AM_FUNC_SAD_ALL_0,  MESEG_BASE,     AM_REG_QWORD,  PECI_CC_SUCCESS},
            { "MESEG_LIMIT",    AM_LOCAL_BUS_1, AM_DEV_CHA_29,  AM_FUNC_SAD_ALL_0,  MESEG_LIMIT,    AM_REG_QWORD,  PECI_CC_SUCCESS},
            { "MMCFG_BASE",     AM_LOCAL_BUS_0, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     MMCFG_BASE,     AM_REG_QWORD,  PECI_CC_SUCCESS},
            { "MMCFG_LIMIT",    AM_LOCAL_BUS_0, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     MMCFG_LIMIT,    AM_REG_QWORD,  PECI_CC_SUCCESS},
            { "MMCFG_Rule",     AM_LOCAL_BUS_1, AM_DEV_CHA_29,  AM_FUNC_UTIL_ALL_1, MMCFG_Rule,     AM_REG_QWORD,  PECI_CC_SUCCESS},
            { "TOLM",           AM_LOCAL_BUS_0, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     TOLM,           AM_REG_DWORD,  PECI_CC_SUCCESS},
            { "TOHM",           AM_LOCAL_BUS_0, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     TOHM,           AM_REG_QWORD,  PECI_CC_SUCCESS},
#else
            { "TSEG0",          AM_LOCAL_BUS_0, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     TSEG,           AM_REG_QWORD, PECI_CC_SUCCESS,	{0} },
            { "TSEG1",          AM_LOCAL_BUS_1, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     TSEG,           AM_REG_QWORD, PECI_CC_SUCCESS,	{0} },
            { "TSEG2",          AM_LOCAL_BUS_2, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     TSEG,           AM_REG_QWORD, PECI_CC_SUCCESS,	{0} },
            { "TSEG3",          AM_LOCAL_BUS_3, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     TSEG,           AM_REG_QWORD, PECI_CC_SUCCESS,	{0} },
            { "MESEG_BASE",     AM_LOCAL_BUS_1, AM_DEV_CHA_29,  AM_FUNC_SAD_ALL_0,  MESEG_BASE,     AM_REG_QWORD, PECI_CC_SUCCESS,	{0} },
            { "MESEG_LIMIT",    AM_LOCAL_BUS_1, AM_DEV_CHA_29,  AM_FUNC_SAD_ALL_0,  MESEG_LIMIT,    AM_REG_QWORD, PECI_CC_SUCCESS,	{0} },
            { "MMCFG_BASE",     AM_LOCAL_BUS_0, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     MMCFG_BASE,     AM_REG_QWORD, PECI_CC_SUCCESS,	{0} },
            { "MMCFG_LIMIT",    AM_LOCAL_BUS_0, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     MMCFG_LIMIT,    AM_REG_QWORD, PECI_CC_SUCCESS,	{0} },
            { "MMCFG_Rule",     AM_LOCAL_BUS_1, AM_DEV_CHA_29,  AM_FUNC_UTIL_ALL_1, MMCFG_Rule,     AM_REG_QWORD, PECI_CC_SUCCESS,	{0} },
            { "TOLM",           AM_LOCAL_BUS_0, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     TOLM,           AM_REG_DWORD, PECI_CC_SUCCESS,	{0} },
            { "TOHM",           AM_LOCAL_BUS_0, AM_DEV_MMAP_5,  AM_FUNC_MMAP_0,     TOHM,           AM_REG_QWORD, PECI_CC_SUCCESS,	{0} },
#endif
        },
		{
		},
    ...
    for (i = 0; i < (sizeof(sAddrMapRawData.sysAddr) / sizeof(SAddrMapReg)); i++) {
		// intialize to 0x40
		sAddrMapRawData.sysAddr[i].u8CC = PECI_CC_SUCCESS;
        ...
        for (u8Dword = 0; u8Dword < sAddrMapRawData.sysAddr[i].u8Dwords; u8Dword++) {
			...
			if (sRdPCIConfigLocalRes.u8CompletionCode != PECI_CC_SUCCESS) {
                PRINT(PRINT_DBG, PRINT_ERROR, "%s (BDF %x:%x:%x reg %x) Local PCI Failed (cc=0x%x)\n", 
					sAddrMapRawData.sysAddr[i].regName, 
					sAddrMapRawData.sysAddr[i].u8Bus, 
					sAddrMapRawData.sysAddr[i].u8Dev, 
					sAddrMapRawData.sysAddr[i].u8Func, 
					sAddrMapRawData.sysAddr[i].u16Reg + (u8Dword * 4),
					sRdPCIConfigLocalRes.u8CompletionCode);

				// The PECI completion code field is intiailized to 0x40. Only override it if we get other than 0x40.
				// That is, always keep the last non-0x40 completion code for display
				sAddrMapRawData.sysAddr[i].u8CC = sRdPCIConfigLocalRes.u8CompletionCode;
            }
        }
    }
    ...
    for (u8Cpu = 0; u8Cpu < MAX_CPU; u8Cpu++) {
        ...
        for (i = 0; i < AM_NUM_MMIO; i++) {
			...
			// intialize to 0x40
			sAddrMapRawData.mmioAddr[u8Cpu][i].u8CC = PECI_CC_SUCCESS;
            ...
            for (u8Dword = 0; u8Dword < AM_REG_QWORD; u8Dword++) {
				...
				if (sRdPCIConfigLocalRes.u8CompletionCode != PECI_CC_SUCCESS) {
                    PRINT(PRINT_DBG, PRINT_ERROR, "CPU #%d %s (BDF %x:%x:%x reg %lx) Local PCI Failed (cc=0x%x)\n", u8Cpu, 
						sAddrMapRawData.mmioAddr[u8Cpu][i].regName, 
						AM_LOCAL_BUS_1, 
						AM_DEV_CHA_29, 
						AM_FUNC_UTIL_ALL_1, 
						MMIO_RULE_CFG_BASE + (i * 8) + (u8Dword * 4),
						sRdPCIConfigLocalRes.u8CompletionCode);

					// The PECI completion code field is intiailized to 0x40. Only override it if we get other than 0x40.
					// That is, always keep the last non-0x40 completion code for display
					sAddrMapRawData.mmioAddr[u8Cpu][i].u8CC = sRdPCIConfigLocalRes.u8CompletionCode;
				}
            }
        }
    }
