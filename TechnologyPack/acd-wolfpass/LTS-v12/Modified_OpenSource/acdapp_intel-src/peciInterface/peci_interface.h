...
#define PECI_CC_MAYBE_SUCCESS   0x91
#define PECI_CC_CMD_PASSED      0x94
#define PECI_CC_DRIVER_ERROR    0xFF
...
typedef struct _SWrPCIConfigLocalReq
{
	SPeciHeader  sHeader;
	unsigned char u8CmdCode;
	unsigned char u8HostID_Retry;
	unsigned char u8PCIConfigAddr[3];
	unsigned char u8Data[4];
	unsigned char u8AWFCS;
}__attribute__((packed)) SWrPCIConfigLocalReq;

typedef struct _SWrPCIConfigLocalRes
{
	unsigned char u8CompletionCode;
}__attribute__((packed)) SWrPCIConfigLocalRes;
#ifndef SPX_BMC_ACD
