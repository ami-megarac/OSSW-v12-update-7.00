...
void handleKbdEvent(int8 down, uint32 key) {
	...
	if ( key >= 'A' && key <= 'Z' && down == 1){
			...
			((prevkey.Modifier & MOD_RIGHT_SHIFT) != MOD_RIGHT_SHIFT))
		{
			...
			if((g_led_status & CAPS_LOCK) != CAPS_LOCK)
			{
				...
				/* Reason for false positive - Passing CAPS_LOCK_KEY_CODE to SendKeyEvent function will not over run the allocated array size*/
	...
	if((key >= NUMPAD_0 && key <= NUMPAD_9) || key == NUMPAD_DECIMAL){
		...
		if((g_led_status & NUM_LOCK) != NUM_LOCK){
			/* Reason for false positive - Passing NUM_LOCK_KEY_CODE to SendKeyEvent function will not over run the allocated array size*/
                        /* coverity[overrun-call : FALSE] */
...
void SendKeyEvent(uint32 key, int8 down){
	...
	/* Reason for false positive - KeybdInfo contains the memory size of both IUSB_HID_PACKET and USB_KEYBD_PKT structure*/
