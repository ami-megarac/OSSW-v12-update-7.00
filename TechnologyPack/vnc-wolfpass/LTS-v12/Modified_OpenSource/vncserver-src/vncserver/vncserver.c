...
static void clientgone(rfbClientPtr cl) {
	...
	if (cl->authComplete == 0) // To send session info if authendication is not completed for non-secure connection
	{
		cmd_info_t comm_info;
		memset(&comm_info, 0, sizeof(cmd_info_t));
		comm_info.cmd = VNC_LOGIN_FAILED;
		comm_info.datalen = INET6_ADDRSTRLEN;
		
		if(WriteDataInAMIVncPipe(&comm_info, (void *)cl->host) < -1){
			printf("\n Error:  Unable to send session info \n");
		}
	}
	...
	if (g_tile_info.compressed_size > 0)
	{
		...
		if (rfbTightEncodingSupported())
			cl->updateBuf[cl->ublen++] = (char)(rfbTightJpeg << 4);
                /* Reason for false positive - Unlocking cl->outputMutex before calling rfbSendCompressedDataTight function will cause functionality break */
                /* coverity[double_lock : FALSE] */
		...
		if (cl->enableLastRectEncoding)
		{
			/* Reason for false positive - Unlocking cl->outputMutex before calling rfbSendLastRectMarker function will cause functionality break */
                        /* coverity[double_lock : FALSE] */
	...
	while (vncpipe != -1) {
		...
		if (read(vncpipe, &cmd, sizeof(cmd)) > 0) {
			...
			switch (cmd.cmd)
			{
				...
				break;
				case RECV_VNC_CONF:
					if((cmd.datalen > 0) && (cmd.datalen <= sizeof(vnc_conf_t)))
					{
						vncConfUpdate = 1;
				...
				case DISCONNECT_VNC_SESSION:
					...
					{
						if((vnc_session.session_index >= 0) && (vnc_session.session_index < vnc_max_session))
						{
...
int WriteDataInAMIVncPipe(cmd_info_t* comm_info, void* data) {
	...
	if(AmiVNCPipe != -1)
	{
		if(comm_info != NULL)
		{
			...
			if ((comm_info->datalen > 0) && (data != NULL))
			{
				if(write(AmiVNCPipe, data, comm_info->datalen) == -1)
				{
					printf("\n session session failed\n");
					return -1;
				}
