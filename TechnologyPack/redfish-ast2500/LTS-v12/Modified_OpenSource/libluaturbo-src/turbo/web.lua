...
function web.Application:_get_request_handlers(request)
    ...
    for i = 1, handlers_sz do
        ...
        if #match > 0 then
            return handler[2], match, handler[3], handler
        end
    end
end
...
function web.Application:__call(request)
    local handler = nil
    local handlers, args, options, route_elem= self:_get_request_handlers(request)
    if handlers then
        -- lazy load if only give handler file path for requiring
        if type(handlers) == "string" then
            local success, reqhandler = pcall(require, handlers)
            if success then
                handlers = reqhandler
                route_elem[2] = reqhandler

                -- Current redfish-resource design will store handler to _G.RESOURCE.handler
                -- Replace the handler string to the real handler
                _G.RESOURCE.handler[route_elem.key] = reqhandler
            else
                log.error(
                    string.format("[web.lua] Error in RequestHandler, require %s failed.", handlers
                        )
                    )
                web.ErrorHandler(
                    self,
                    request,
                    500,
                    string.format('<pre style="font-size:12px; font-family:monospace; color:#8B0000;">[web.lua] Error in RequestHandler, require %s failed.</pre>',
                        handlers
                        )
                    )
                return
